...
var mysqlLostUpdateHack = &cobra.Command{
	Use:   "mysql_lost_update_hack <invoice_id> <amount> <version>",
...
func MysqlLostUpdateHack(_ *cobra.Command, args []string) (err error) {
	if len(args) != 3 {
		return errors.New("Required 3 args: 1) invoiceID, 2) Amount, 3) Version ")
	}
	dbName := "mysql"
	...
	if err = tests.MySQLLostUpdateHack(ctx, db, txLevel, dbName, invoiceID, amount, version); err != nil {
